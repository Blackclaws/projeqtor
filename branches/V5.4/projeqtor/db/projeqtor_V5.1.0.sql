...
ADD `idDeliveryMode` int(12) unsigned DEFAULT null;
ALTER TABLE `${prefix}command` CHANGE `initialAmount` `untaxedAmount` DECIMAL(12,2) UNSIGNED;
ALTER TABLE `${prefix}command` CHANGE `addAmount` `addUntaxedAmount` DECIMAL(12,2) UNSIGNED;
ALTER TABLE `${prefix}command` CHANGE `validatedAmount` `totalUntaxedAmount` DECIMAL(12,2) UNSIGNED;
